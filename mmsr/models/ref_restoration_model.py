...
class RefRestorationModel(SRModel):
    ...
    def optimize_parameters(self, step):
        ...
        features_ref_hrp = self.net_extractor(self.match_img_ref_hrp, self.output)
        pre_offset_ref_hrp, sr_feat = self.net_map(
            features_ref_hrp, self.output)
        out_ref_hrp = self.net_g(self.img_ref_hrp_lq, pre_offset_ref_hrp,
                                 sr_feat)
    ...
    def test(self, original_size=None):
        self.net_g.eval()
        with torch.no_grad():
            self.features = self.net_extractor(self.match_img_in, self.img_ref)
            self.pre_offset, self.img_ref_feat = self.net_map(
                self.features, self.img_ref)
            self.output = self.net_g(self.img_in_lq, self.pre_offset,
                                     self.img_ref_feat, original_size)
        self.net_g.train()
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger,
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            self.test(original_size if padding else None)
